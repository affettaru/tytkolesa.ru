...
<?
	if (true === $arResult["bDescPageNumbering"])
	{
		...
		{
			?><a class="pagination__link" href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>&SIZEN_<?=$arResult["NavNum"]?>=<?=$arResult['NavPageSize']; ?>" title="<? echo GetMessage('nav_prev_title'); ?>"> <div class="pagination__item"><button class="pagination__btn" ><svg>
            <use xlink:href="<?=SITE_TEMPLATE_PATH?>/img/icons.svg#ic-arrowleft-big"></use>
        </svg></button></div></a><?
		}
		else
		...
		while ($NavRecordGroup >= 1)
		{
			...
			{
				?><li class="pagination__link pagination__link__active" title="<? echo GetMessage('nav_page_current_title'); ?>"><? echo $NavRecordGroupPrint; ?></li><?
			}
...
?>
		...
		{
			?><a href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]-1)?>&SIZEN_<?=$arResult["NavNum"]?>=<?=$arResult['NavPageSize']; ?>" title="<? echo GetMessage('nav_prev_title'); ?>"> <div class="pagination__item"><button class="pagination__btn" ><svg>
            <use xlink:href="<?=SITE_TEMPLATE_PATH?>/img/icons.svg#ic-arrowleft-big"></use>
        </svg></button></div></a><?
		}
		else
		...
		while($NavRecordGroup <= $arResult["NavPageCount"])
		{
			...
			{
				?>
                <li><a class="pagination__link pagination__link__active" href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=$NavRecordGroup?>&SIZEN_<?=$arResult["NavNum"]?>=<?=$arResult['NavPageSize']; ?>" title="<? echo $strTitle; ?>"><?=$NavRecordGroup?></a></li>
                <?
			}
		...
		if ($arResult["NavPageNomer"] < $arResult["NavPageCount"])
		{
			?><div class="pagination__item"><a  href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>&SIZEN_<?=$arResult["NavNum"]?>=<?=$arResult['NavPageSize']; ?>" title="<? echo GetMessage('nav_next_title'); ?>"><button class="pagination__btn"><svg>
            <use xlink:href="<?=SITE_TEMPLATE_PATH?>/img/icons.svg#ic-arrowright-big"></use>
        </svg></button></a></div><?
		}
		else
